...
public class Uninformer : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        var padding = Mathf.Max(0, maxLights - count);

        intensities  =  intensities.Concat(Enumerable.Repeat(0f, padding)).Take(maxLights);
        colors       =       colors.Concat(Enumerable.Repeat(Vector4.zero, padding)).Take(maxLights);
        positions    =    positions.Concat(Enumerable.Repeat(Vector4.zero, padding)).Take(maxLights);
        forwards     =     forwards.Concat(Enumerable.Repeat(Vector4.zero, padding)).Take(maxLights);
        coneCosines  =  coneCosines.Concat(Enumerable.Repeat(0f, padding)).Take(maxLights);
        coneFalloffs = coneFalloffs.Concat(Enumerable.Repeat(0f, padding)).Take(maxLights);
        attenuations = attenuations.Concat(Enumerable.Repeat(2f, padding)).Take(maxLights);
        types        =        types.Concat(Enumerable.Repeat(0f, padding)).Take(maxLights);
